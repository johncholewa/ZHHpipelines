...
if [ ${3} == "TMS" ]; then
  ...
  docker run -i --rm -v ${rawloc}/${2}/dicom/:/input/dicom/${1}/${2}:ro -v ${BIDS}/:/output -v ${MY_FULL_PATH}:/derivatives -v ${TRASH}:/work  amiklos/bidskit:2.1 --indir=/input/dicom --outdir=/output #--overwrite
